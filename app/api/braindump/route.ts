...
- title에는 사용자가 쓴 표현과 낱말을 최대한 그대로 살리세요. 사용자가 자기가 적은 일을 한눈에 알아볼 수 있어야 해요. 매끄럽게 다듬느라 원래 쓰던 말을 바꾸지 말고, 입력에 없는 일을 새로 지어내지도 마세요.
...
- 이때 예시는 반드시 사용자가 방금 적은 주제를 그대로 이어서, 그 주제를 조금 더 구체적으로 적으면 어떤 모습이 되는지 보여주세요. 주제와 상관없는 일반적인 예시를 주면 사용자는 자기 이야기가 무시당했다고 느껴요. (예: 입력이 "시험 공부 뭐하지"라면 → "내일 시험 과목이 뭔지부터 정리해야 해", "1주차 강의노트를 아직 못 봤어"처럼 같은 주제로)
...
function braindumpUser(braindump: string): string {
  return `[브레인덤프]\n${braindump}\n\n위 내용에서 실행 가능한 할 일 후보를 빠짐없이, 최대 10개까지 전부 뽑아 주세요. 미리 추려주지 말고 찾은 만큼 다 보여주세요 — 고르는 건 다음 단계에서 사용자가 해요. 각 후보는 위 글에 쓰인 표현을 최대한 그대로 살려, 사용자가 자기가 적은 일임을 바로 알아볼 수 있게 써 주세요. 후보를 만들기 어려운 입력이면 위와 같은 주제를 이어가는 구체적인 예시 2~3개로 다시 쏟아내도록 도와주세요.`;
}
