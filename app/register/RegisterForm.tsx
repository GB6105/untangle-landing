    ...
    <form action={formAction} className="flex flex-col gap-7 pt-7">
      ...
      <div className="flex flex-col gap-2.5">
        ...
        {/* 수집 항목·목적·보유기간을 적는 지점. 연락처는 선택 항목이라 별도
            동의 체크박스 대신 고지와 링크로 갈음한다. 인터뷰에 체크하면 이용
            목적이 늘고 "비워도 된다"는 안내가 거짓이 되므로 문구를 바꾼다.
            처리방침은 새 탭으로 열어 작성 중인 소감이 날아가지 않게 한다. */}
        <p className="text-[12px] leading-[1.5] text-sys-label-alt">
          {interview
            ? "적어주신 연락처는 출시 안내와 인터뷰 연락에만 쓰고, 쓰임을 다한 뒤 바로 지워요."
            : "적어주신 연락처는 출시 안내에만 쓰고, 안내를 보낸 뒤 바로 지워요. 비워두셔도 괜찮아요."}{" "}
          <Link
            href="/privacy"
            target="_blank"
